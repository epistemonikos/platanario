...
def calculate_textrank_for_file(input_path, stopwords=None):
    ...
    for group_id in text_rank.groups_phrases:
        for phrase_data in sorted(
                text_rank.groups_phrases[group_id],
                key=lambda x: x.get("count") * x.get("rank"),
                reverse=True
            ):
            if phrase_data.get("text") in EN_STOPWORDS:
                continue
            print_row(
                [
...
def main():
    ...
    calculate_textrank_for_file(file_path)
